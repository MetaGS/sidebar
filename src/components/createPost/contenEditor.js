...
var ContentEditable = /** @class */ (function (_super) {
    ...
    ContentEditable.prototype.shouldComponentUpdate = function (nextProps) {
        ...
        if (!el) {
            console.log('127 shouldComponentUpdate')
            return true;
        }
        // ...or if html really changed... (programmatically, not by user edit)
        if (normalizeHtml(nextProps.html) !== normalizeHtml(el.innerHTML)) {
            const htmlFromProps = normalizeHtml(nextProps.html);
            const htmlFromInner = normalizeHtml(el.innerHTML);
            console.log(`
                htmlFromProps ${htmlFromProps}
                htmlFromInner ${htmlFromInner}
            `)
            console.log('131 shouldComponentUpdate')
